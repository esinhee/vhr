<template>
  <div>
    <div style="text-align: left">
      <el-button type="primary" icon="el-icon-plus" size="mini" @click="newTicket">新建工单</el-button>
    </div>
  ...
  export default{
    ...
    methods: {
      ...
      loadTableData(){
        var _this = this;
        this.loading = true;
        this.getRequest("/ticket/tl?page=" + this.currentPage + "&size=10").then(resp=> {
          _this.loading = false;
          if (resp && resp.status == 200) {
            var data = resp.data;
      ...
      reassign(row) {
        //分配按钮显示，
        if (this.user.id == row.recorderid && this.iDuty == 1 && row.dealflg < 7) {
          return true;
        } else {
          return false;
        }
